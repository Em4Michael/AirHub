...
// Route: /dashboard/analyst/admin/accounts/page.tsx
...
import {
  Search, Briefcase, ChevronLeft, ChevronRight, ChevronRight as Arrow,
  BarChart2,
} from 'lucide-react';
    ...
    <div className="space-y-6 pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-black" style={{ color: 'var(--text-primary)' }}>Accounts</h1>
          </div>
          <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>
            {total} account{total === 1 ? '' : 's'} · tap any to see production and per-worker hours
          </p>
        </div>
        <Link href="/dashboard/analyst/admin/production"
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2">
          {([['', 'All'], ['available', 'Available'], ['claimed', 'Claimed']] as [StatusFilter, string][]).map(([val, label]) => (
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border overflow-hidden" style={{ borderColor: 'var(--border-color)' }}>
          <div className="divide-y" style={{ borderColor: 'var(--border-color)' }}>
            {rows.map((a) => (
              <Link key={a._id} href={`/dashboard/analyst/admin/accounts/${a._id}`}
      ...
      {pages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
            className="p-2 rounded-xl disabled:opacity-30" style={{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text-secondary)', border: '1px solid var(--border-color)' }}>
